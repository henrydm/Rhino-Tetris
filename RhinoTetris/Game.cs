...
namespace RhinoTetris
{
    internal class Game
    {
        ...
        private WindowsMediaPlayer[] _fxPlayers;
        ...
        private void _playerMusic_MediaEnded(object sender, EventArgs e)
        {
            if (!Music) return;
            _playerMusic = new MediaPlayer();
            _playerMusic.Open(new Uri(_musicPath));
        ...
        private void GameFinished(object sender, RunWorkerCompletedEventArgs e)
        {
            _playerMusic.Stop();
            SetDownScene();
            RhinoDoc.ActiveDoc?.Views?.Redraw();
        ...
        private void PlayFx(string uri)
        {
            if (Fx) return;
            _playerCurrent++;
